...
tasks:
  ...
  $applicationName:install:sandbox:
    desc: "Top Level Task to Install Gitlab Sandbox"
    deps: [core:setup]
    cmds:
  ...
  $applicationName:uninstall:sandbox:
    desc: "Top Level Task to Uninstall Gitlab Sandbox"
    cmds:
      - task: terraform:destroy:sandbox
    silent: true

  $applicationName:reinstall:sandbox:
    desc: "Top Level Task to Re-Install Gitlab Sandbox"
    cmds:
  ...
  $applicationName:install:labor:
    desc: "Top Level Task to Install Gitlab Labor"
    deps: [core:setup]
    cmds:
  ...
  $applicationName:uninstall:labor:
    desc: "Top Level Task to Uninstall Gitlab Labor"
    cmds:
      - task: terraform:destroy:labor
    silent: true

  $applicationName:reinstall:labor:
    desc: "Top Level Task to Re-Install Gitlab Labor"
    cmds:
  ...
  $applicationName:install:production:
    desc: "Top Level Task to Install Gitlab Production"
    deps: [core:setup]
    cmds:
      ...
      - echo "Sleeping for 60 seconds to wait for the VM to be ready"
      - echo "Sleeping for 30 seconds to wait for the VM to be ready"
      - sleep 30
  ...
  $applicationName:uninstall:production:
    desc: "Top Level Task to Uninstall Gitlab Production"
    cmds:
      - task: terraform:destroy:production
    silent: true

  $applicationName:reinstall:production:
    desc: "Top Level Task to Re-Install Gitlab Production"
    cmds:
  ...
  vault:get:private_key:sandbox:
    ...
    cmds:
      - task: _vault:get:private_key
        vars: { GITLAB_VM_NAME: '{{.GITLAB_VM_NAME_SANDBOX}}', GITLAB_VM_FQDN: '{{.GITLAB_VM_FQDN_SANDBOX}}', MOUNT_PATH: '{{.VAULT_MOUNT_PATH_SANDBOX}}' }
  ...
  vault:get:private_key:labor:
    ...
    cmds:
      - task: _vault:get:private_key
        vars: { GITLAB_VM_NAME: '{{.GITLAB_VM_NAME_LABOR}}', GITLAB_VM_FQDN: '{{.GITLAB_VM_FQDN_LABOR}}', MOUNT_PATH: '{{.VAULT_MOUNT_PATH_LABOR}}' }
  ...
  vault:get:private_key:production:
    ...
    cmds:
      - task: _vault:get:private_key
        vars: { GITLAB_VM_NAME: '{{.GITLAB_VM_NAME_PRODUCTION}}', GITLAB_VM_FQDN: '{{.GITLAB_VM_FQDN_PRODUCTION}}', MOUNT_PATH: '{{.VAULT_MOUNT_PATH_PRODUCTION}}' }
  ...
  _vault:get:private_key:
    ...
    vars:
      GITLAB_VM_NAME: '{{default "" .GITLAB_VM_NAME}}'
      GITLAB_VM_FQDN: '{{default "" .GITLAB_VM_FQDN}}'
      MOUNT_PATH: '{{default "" .MOUNT_PATH}}'
    cmds:
      # download from vault
      - vault kv get -field=openssh_private -mount={{.MOUNT_PATH}} {{.GITLAB_VM_NAME}}/ssh_ansible > {{.GITLAB_MGM_PLAYBOOK_DIR}}/ssh/{{.GITLAB_VM_NAME}}_ansible_ed25519
      # make executable
      - chmod 600 {{.GITLAB_MGM_PLAYBOOK_DIR}}/ssh/{{.GITLAB_VM_NAME}}_ansible_ed25519
      # remove from known_hosts
      - ssh-keygen -f "{{.HOME}}/.ssh/known_hosts" -R "[{{.GITLAB_VM_FQDN}}]:22022"
      # Make sure these variables are set before running
    requires:
      vars: [ GITLAB_VM_NAME, GITLAB_VM_FQDN, MOUNT_PATH ]
    silent: true
  ...
  ssh:test:sandbox:
    ...
    cmds:
      - task: _ssh:test
        vars: { GITLAB_VM_NAME: '{{.GITLAB_VM_NAME_SANDBOX}}', GITLAB_VM_FQDN: '{{.GITLAB_VM_FQDN_SANDBOX}}' }
    silent: true
  ...
  ssh:test:labor:
    ...
    cmds:
      - task: _ssh:test
        vars: { GITLAB_VM_NAME: '{{.GITLAB_VM_NAME_LABOR}}', GITLAB_VM_FQDN: '{{.GITLAB_VM_FQDN_LABOR}}' }
    silent: true
  ...
  ssh:test:production:
    ...
    cmds:
      - task: _ssh:test
        vars: { GITLAB_VM_NAME: '{{.GITLAB_VM_NAME_PRODUCTION}}', GITLAB_VM_FQDN: '{{.GITLAB_VM_FQDN_PRODUCTION}}' }
    silent: true
  ...
  _ssh:test:
    ...
    vars:
      ...
      GITLAB_VM_FQDN: '{{default "" .GITLAB_VM_FQDN}}'
    cmds:
      - ssh -o StrictHostKeyChecking=no -p 22022 -i {{.GITLAB_MGM_PLAYBOOK_DIR}}/ssh/{{.GITLAB_VM_NAME}}_ansible_ed25519 ansible@{{.GITLAB_VM_FQDN}} "date"
    silent: true
  ...
  _ssh:connect:
    ...
    vars:
      ...
      GITLAB_VM_FQDN: '{{default "" .GITLAB_VM_FQDN}}'
    cmds:
      - ssh -o StrictHostKeyChecking=no -p 22022 -i {{.GITLAB_MGM_PLAYBOOK_DIR}}/ssh/{{.GITLAB_VM_NAME}}_ansible_ed25519 ansible@{{.GITLAB_VM_FQDN}}
    silent: true
  ...
  ansible:test:sandbox:
    ...
    cmds:
      - task: _ansible:test:connectivity
        vars:
          VARIABLE_HOST: '{{.GITLAB_VM_FQDN_SANDBOX}}'
          VARIABLE_STAGE: 'staging'
    silent: true
  ...
  ansible:test:labor:
    ...
    cmds:
      - task: _ansible:test:connectivity
        vars:
          VARIABLE_HOST: '{{.GITLAB_VM_FQDN_LABOR}}'
          VARIABLE_STAGE: 'staging'
    silent: true
  ...
  ansible:test:production:
    ...
    cmds:
      - task: _ansible:test:connectivity
        vars:
          VARIABLE_HOST: '{{.GITLAB_VM_FQDN_PRODUCTION}}'
          VARIABLE_STAGE: 'production'
    silent: true

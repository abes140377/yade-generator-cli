...
if [[ -f "$file" ]]; then
    ...
    if grep -q "<server>:<port>" "$file"; then
        echo ""
        echo "⚠ Fehler:"
        echo "In der Datei '$file' befindet sich noch der Platzhalter '<server>:<port>'."
    ...
    if grep -q "<vault_token>" "$file"; then
        echo ""
        echo "⚠ Fehler:"
        echo "In der Datei '$file' befindet sich noch der Platzhalter '<vault_token>'."
    ...
    echo "⚠ Fehler:"
    echo "Die Datei '$file' existiert nicht im aktuellen Verzeichnis."
